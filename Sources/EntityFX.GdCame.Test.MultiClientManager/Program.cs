...
namespace EntityFX.GdCame.Test.MultiClientManager
{
    internal class Program
    {
        ...
        private static readonly ConcurrentDictionary<string, Guid> _testUsersDisctionary =
        ...
        private static void Main(string[] args)
        {
            ...
            var testNumber = 1;
            int.TryParse(testNumberString, out testNumber);
        ...
        private static void TestMulticlientPerformance()
        {
            ...
            }.ToList().ForEach(t =>
            {
                ...
                sw.Start();
                Parallel.ForEach(_testUsersDisctionary,
                    (pair, state) => Parallel.For(0, 50, _ => GetGameClient(pair.Value).Ping()));
                sw.Stop();
                Console.WriteLine("{0}: {1}", t, sw.Elapsed);
                Parallel.ForEach(_testUsersDisctionary, (pair, state) =>
                {
        ...
        private static void StartMultipleClients()
        {
            ...
            int.TryParse(sessionsNumberString, out sessionsNumber);
            var watches = new List<long>();
            ...
            Console.WriteLine("Total: {0}, Avg={1}, Min={2}, Max={3}", sw2.ElapsedMilliseconds, watches.Average(),
                watches.Min(), watches.Max());
            sw2.Stop();
            watches.Clear();
        }
