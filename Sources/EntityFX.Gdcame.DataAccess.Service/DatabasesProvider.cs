...
namespace EntityFX.Gdcame.DataAccess.Service
{
        public class DatabasesProvider:IDatabasesProvider
    {
        ...
        public int addUser(string serverId, User user)
        {
            int userId = 0;
            UserRepository currentUserRepository = null;
            bool repoExists=false;

            if (!userRepositories.ContainsKey(serverId))
            {
                currentUserRepository = getUserRepository(serverId);
                userRepositories.Add(serverId, currentUserRepository);
            }
            
            repoExists = userRepositories.TryGetValue(serverId, out currentUserRepository);
            if (repoExists)
            {
                try
                {
                    userId = currentUserRepository.Create(user);
                }
                catch (Exception e)
                {
                    Console.WriteLine(String.Format("User {0} was not added:{1}",
                         user, e));
                }
            ...
            {
                Console.WriteLine(String.Format("Cannot get repository for server {0}. User {1} is not created",
                    serverId, user));
            }
